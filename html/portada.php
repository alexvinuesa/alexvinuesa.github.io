<!DOCTYPE html>
<html lang="es">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>SOCIAL CONNECTION</title>
    <link rel="stylesheet" href="../css/style.css">
    <script src="../js/bootstrap.bundle.min.js"></script>
    <link rel="stylesheet" href="../css/bootstrap.min.css">
    <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
    <style>
        #gastronomia {
            background-image: url(../images/fotooo.jpg);
        }
        .text{
            margin-left:95px;
        }
    </style>
</head>

<body>

    <header id="portada-principal">
        <?php session_start();?>

        <nav class="navbar navbar-expand-sm navbar-dark" id="menu">
            <div class="container">
                <a class="navbar-brand" href="portada.php">
                    <img id="logo" src="../images/logo.png" alt="" width="150" height="94">
                </a>
                <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav"
                    aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="bx bx-grid-alt"></span>
                </button>
                <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
                    <ul class="navbar-nav">

                        <li class="nav-item ms-4">
                            <a id="menu" class="nav-link active" aria-current="page" href="#servicios">NUESTROS
                                SERVICIOS</a>
                        </li>
                        <li class="nav-item ms-4">
                            <a id="menu" class="nav-link active" href="#quienes-somos">QUIENES SOMOS</a>
                        </li>
                        <li class="nav-item ms-4 me-4">
                            <a id="menu" class="nav-link active" href="#formulario">CONTACTO</a>
                        </li>
                        <div class="boton-registrarse">
                            <li class="nav-item">
                                <?php
                                if (!isset($_SESSION['username'])) {
                                    echo '<a id="hover" class="nav-link active" href="login.php">INICIAR SESSIÓN</a>';
                                } else {
                                    echo '<a id="hover" class="nav-link active" href="login.php">CERRAR SESIÓN</a>';
                                    // Destruir la sesión
                                    session_destroy();
                                }
                                ?>
                            </li>
                        </div>
                    </ul>
                </div>
            </div>
        </nav>
        <div class="container header">
            <div class="row">
                <h6 >SOCIAL CONNECTION</h6>
                <h1 >La vida es una <span style="color: white;">FIESTA</span><br>
                    ¡Únete a la nuestra!</h1>
                <p style="width: 450px;">
                    La página web ofrece una amplia variedad de actividades deportivas y de ocio nocturno para
                    disfrutar en tu ciudad. ¡No te pierdas la oportunidad de disfrutar al máximo de tu tiempo libre con
                    nosotros!
                </p>

            </div>
            <a class="nav-link active" id="buscar" href="#quienes-somos">MAS INFORMACIÓN</a>
        </div>
    </header>
    <div class="container text-center mt-5" id="servicios">
        <div class="row justify-content-center">
            <h2>NUESTROS SERVICIOS</h2>
            <div class="col-3 text-center" id="deporte">
                <a href="deportes.php">
                    <h6 id="servicio" class="text-white">DEPORTE</h6>
                </a>
            </div>

            <div class="col-3" id="ocio-nocturno">
                <a href="fiesta.php">
                    <h6 id="servicio" class="text-white">OCIO NOCTURNO</h6>
                </a>
            </div>


            <div class="col-3" id="gastronomia">
                <a href="gastronomia.php">
                    <h6 id="servicio" class="text-white">GASTRONOMIA</h6>
                </a>
            </div>

        </div>
    </div>

    <div class="container mt-5" id="quienes-somos">
        <div class="row">
            
            <div class="col-6">
                <h2 class="text">QUIENES SOMOS</h2>
                <p style="width: 530px;" class="text">La página web ofrece una amplia variedad de actividades deportivas y
                    de ocio nocturno para disfrutar en tu ciudad. ¡No te pierdas la oportunidad
                    de disfrutar al máximo de tu tiempo libre con nosotros!La página web ofrece una
                    amplia variedad de actividades deportivas y de ocio nocturno para disfrutar
                    en tu ciudad. ¡No te pierdas la oportunidad de disfrutar al máximo de tu tiempo
                    libre con nosotros!La página web ofrece una amplia variedad de actividades
                    deportivas y de ocio nocturno para disfrutar en tu ciudad. ¡No te pierdas la
                    oportunidad de disfrutar al máximo de tu tiempo libre con nosotros!
                </p>
            </div>
            <div class="col-6 text-center">
                <img src="../images/quienes-somos.png" alt="">
            </div>
        </div>
    </div>

    <div class="container-fluid" id="clientes">
        <div class="row">
            <h1>NUESTROS CLIENTES</h1>
            <div class="cliente1"></div>
        </div>
    </div>

    <div class="container-fluid" id="formulario">
        <div class="row justify-content-center ">
            <div id="form" class="col-5 shadow">
                <form>
                    <h1 class="text-center">Contacta con nosotros</h1>
                    <div class="row mt-4">
                        <div class="form-group col-6">
                            <label for="nombre">Nombre:</label>
                            <input type="text" class="form-control" id="nombre" placeholder="Ingresa tu nombre"
                                required>
                        </div>
                        <div class="form-group col-6">
                            <label for="apellido">Apellido:</label>
                            <input type="text" class="form-control" id="apellido" placeholder="Ingresa tu apellido"
                                required>
                        </div>
                        <div class="form-group mt-3">
                            <label for="email">Teléfono:</label>
                            <input type="email" class="form-control" id="email" placeholder="Ingresa tu teléfono"
                                required>
                        </div>
                        <div class="form-group mt-3">
                            <label for="email">Email:</label>
                            <input type="email" class="form-control" id="email" placeholder="Ingresa tu email" required>
                        </div>
                        <div class="form-group mt-3">
                            <label for="mensaje">Mensaje:</label>
                            <textarea class="form-control" id="mensaje" rows="5" placeholder="Escribe tu mensaje"
                                required></textarea>
                        </div>
                        <center><button type="submit" id="boton-formulario" class="btn btn-primary mt-4">Enviar</button>
                        </center>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <div class="container-fluid" id="footer">
        <div class="row justify-content-center text-white">
            <div class="col-3">
                <img src="../images/telefono-icono.png" width="30px" alt=""><span class="text-white"> Teléfono</span>
                <p>654 78 96 54</p>
                <p>684 23 89 73</p>
            </div>
            <div class="col-3">
                <img src="../images/correo-icono.png" width="30px" alt=""><span class="text-white"> Correo
                    electrónico</span><br>
                <p>soporte@socialconnection.com</p>
                <p>administracion@socialconnection.com</p>
            </div>
            <div class="col-3">
                <img src="../images/ubicacion-icono.png" width="30px" alt=""><span class="text-white"> Ubicación</span>
                <p>Carrer de Lles, 3, 08207 Sabadell, Barcelona </p>
            </div>
            <div class="col-12">
                <p class="text-center mt-3">© Derechos de autor Social Connection Página Oficial del Social Connection</p>
            </div>
        </div>
    </div>


</body>

</html>
