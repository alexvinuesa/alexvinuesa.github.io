...
<html>
  ...
  <body>
    ...
    <h1>Crear evento comida</h1>
    <form action="../php/creareventocom.php" enctype="multipart/form-data" method="post">
  ...
  <label for="tipocom">Tipo comida:</label>
  ...
  <?php
    ...
    $sql = "SELECT * FROM nomcomida";
    $result = mysqli_query($conn, $sql);

    // Generación de las opciones del campo de selección
    while($row = mysqli_fetch_assoc($result)) {
      echo "<option value=\"" . $row["tipocom"] . "\">" . $row["tipocom"] . "</option>";
    }
  ...
  <label for="imatge">Imatge:</label><br><br>
